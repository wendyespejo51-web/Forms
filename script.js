document.addEventListener("DOMContentLoaded", () => {
  try {
    // --- Obtener elementos (con tolerancia a diferentes ids de formulario) ---
    const form = document.getElementById("registroForm") || document.getElementById("RegistroInspeccionToroidal");
    const enviarBtn = document.getElementById("enviarBtn");
    if (!form) {
      console.error("No se encontró el formulario (id='registroForm' o 'RegistroInspeccionToroidal').");
      return;
    }
    if (!enviarBtn) {
      console.error("No se encontró el botón de enviar (id='enviarBtn').");
      return;
    }

    // --- SED / suggestions ---
    const inputSED = document.getElementById("codigo");
    if (!inputSED) console.warn("No se encontró input 'codigo' (SED).");

    let suggestionsList = document.getElementById("suggestions");
    if (!suggestionsList && inputSED) {
      // crear UL para sugerencias si no existe
      suggestionsList = document.createElement("ul");
      suggestionsList.id = "suggestions";
      suggestionsList.style.display = "none";
      suggestionsList.className = "suggestions-list";
      inputSED.parentNode.appendChild(suggestionsList);
    }

    // Lista de SED (ejemplo; reemplaza por tus 1033)
    const listaSED = [
      "00021S",
      "00023S",
      "00028S",
    ...
    // --- Autocompletado SED ---
    if (inputSED && suggestionsList) {
      inputSED.addEventListener("input", function () {
        const valor = inputSED.value.trim().toUpperCase();
    ...
    // cerrar sugerencias al click fuera o ESC
    document.addEventListener("click", function (e) {
      if (suggestionsList && inputSED && e.target !== inputSED && !suggestionsList.contains(e.target)) {
        suggestionsList.style.display = "none";
      }
    });
    if (inputSED) {
      inputSED.addEventListener("keydown", function (e) {
        if (e.key === "Escape" && suggestionsList) suggestionsList.style.display = "none";
      });
    }

    // --- Mensaje visible (crear si no existe) ---
    ...
    if (!mensajeDiv) {
      ...
      // insertarlo justo después del botón enviar si existe
      const afterEl = enviarBtn;
      afterEl.insertAdjacentElement("afterend", mensajeDiv);
    }

    // --- Handler click enviar ---
    enviarBtn.addEventListener("click", async function (e) {
      e.preventDefault();

      try {
        // Validación campos required
        const requiredFields = form.querySelectorAll("[required]");
        const missingFields = [];
        requiredFields.forEach(field => {
          // Para inputs tipo file o checkbox, se puede ajustar si es necesario
          const value = (field.type === "file") ? field.files.length : (field.value ? field.value.trim() : "");
          if (!value) {
        ...
        if (missingFields.length > 0) {
          ...
          mensajeDiv.style.background = "#fdecea";
          mensajeDiv.style.padding = "8px";
          mensajeDiv.style.borderRadius = "6px";
          return;
        }

        // Construir objeto datos (tratamos 'codigo' como texto)
        const datos = {};
        const campos = [
        ...
        if (archivosInput && archivosInput.files) {
        ...
        mensajeDiv.style.background = "#2f86ff";
        mensajeDiv.style.padding = "8px";
        mensajeDiv.style.borderRadius = "6px";

        // SEND: reemplaza aquí tu URL real
        ...
        // Aceptar 200,201,202 como éxito
        ...
        // Cualquier error queda visible y en consola
        mensajeDiv.textContent = "⚠️ Ocurrió un error interno al enviar. Revisa consola.";
        mensajeDiv.style.background = "#fdecea";
        mensajeDiv.style.color = "crimson";
    ...
    // si llegamos hasta aquí, la inicialización fue exitosa
    console.log("Script inicializado correctamente.");
  } catch (initErr) {
    console.error("Error inicializando script:", initErr);
  }
});
